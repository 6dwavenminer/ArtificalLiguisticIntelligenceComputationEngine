...
int OLD_SaveNeuralNetworkModule(
	...
	VectorToSave.push_back(18);		//NumOfVectorsToSave
									//Size of all the vectors
	VectorToSave.push_back((*NeuralNetworkToSave).BufferNodeValues.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMInputNodeIDBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMOutputNodeIDBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMWeightsListBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMPreviousCellStateValueBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMPreviousOutputValueBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMForgetGateValueBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMInputGateValueBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMCandidateValueBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMOutputGateValueBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMCellStateValueBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).LSTMOutputValueBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).NeuronNodeStartBuffer.size());
	VectorToSave.push_back((*NeuralNetworkToSave).NeuronNodeEndBuffer.size());
	...
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMInputNodeIDBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMInputNodeIDBuffer[i]);
	}
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMOutputNodeIDBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMOutputNodeIDBuffer[i]);
	}
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMWeightsListBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMWeightsListBuffer[i]);
	}
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMPreviousCellStateValueBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMPreviousCellStateValueBuffer[i]);
	}
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMPreviousOutputValueBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMPreviousOutputValueBuffer[i]);
	}
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMForgetGateValueBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMForgetGateValueBuffer[i]);
	}
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMInputGateValueBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMInputGateValueBuffer[i]);
	}
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMCandidateValueBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMCandidateValueBuffer[i]);
	}
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMOutputGateValueBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMOutputGateValueBuffer[i]);
	}
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMCellStateValueBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMCellStateValueBuffer[i]);
	}
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).LSTMOutputValueBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).LSTMOutputValueBuffer[i]);
	}
	...
	for (unsigned int i = 0; i < (*NeuralNetworkToSave).NeuronNodeEndBuffer.size(); i++) {
		VectorToSave.push_back((float)(*NeuralNetworkToSave).NeuronNodeEndBuffer[i]);
	}
...
int OLD_LoadNeuralNetworkModule(
	...
	(NeuralNetworkToLoad.LSTMInputNodeIDBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[2]; i++) {
		(NeuralNetworkToLoad.LSTMInputNodeIDBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[2];

	(NeuralNetworkToLoad.LSTMOutputNodeIDBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[3]; i++) {
		(NeuralNetworkToLoad.LSTMOutputNodeIDBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[3];

	(NeuralNetworkToLoad.LSTMWeightsListBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[4]; i++) {/////
		(NeuralNetworkToLoad.LSTMWeightsListBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[4];

	(NeuralNetworkToLoad.LSTMPreviousCellStateValueBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[5]; i++) {//
		(NeuralNetworkToLoad.LSTMPreviousCellStateValueBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[5];

	(NeuralNetworkToLoad.LSTMPreviousOutputValueBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[6]; i++) {
		(NeuralNetworkToLoad.LSTMPreviousOutputValueBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[6];

	(NeuralNetworkToLoad.LSTMForgetGateValueBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[7]; i++) {
		(NeuralNetworkToLoad.LSTMForgetGateValueBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[7];

	(NeuralNetworkToLoad.LSTMInputGateValueBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[8]; i++) {
		(NeuralNetworkToLoad.LSTMInputGateValueBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[8];

	(NeuralNetworkToLoad.LSTMCandidateValueBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[9]; i++) {
		(NeuralNetworkToLoad.LSTMCandidateValueBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[9];

	(NeuralNetworkToLoad.LSTMOutputGateValueBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[10]; i++) {
		(NeuralNetworkToLoad.LSTMOutputGateValueBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[10];

	(NeuralNetworkToLoad.LSTMCellStateValueBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[11]; i++) {
		(NeuralNetworkToLoad.LSTMCellStateValueBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[11];

	(NeuralNetworkToLoad.LSTMOutputValueBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[12]; i++) {
		(NeuralNetworkToLoad.LSTMOutputValueBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[12];

	(NeuralNetworkToLoad.NeuronNodeStartBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[13]; i++) {
		(NeuralNetworkToLoad.NeuronNodeStartBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[13];

	(NeuralNetworkToLoad.NeuronNodeEndBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[14]; i++) {
		(NeuralNetworkToLoad.NeuronNodeEndBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[14];

	(NeuralNetworkToLoad.NeuronInputNodeIDsBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[15]; i++) {
		(NeuralNetworkToLoad.NeuronInputNodeIDsBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[15];

	(NeuralNetworkToLoad.NeuronOutputNodeIDsBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[16]; i++) {//15
		(NeuralNetworkToLoad.NeuronOutputNodeIDsBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[16];

	(NeuralNetworkToLoad.NeuronOutputNodeBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[17]; i++) {//16
		(NeuralNetworkToLoad.NeuronOutputNodeBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[17];

	(NeuralNetworkToLoad.NeuronWeightsBuffer).clear();
	for (unsigned int i = 0; i < VectorToLoad[18]; i++) {//19
		(NeuralNetworkToLoad.NeuronWeightsBuffer).push_back(VectorToLoad[VecPos + i]);
	}
	VecPos += VectorToLoad[18];
